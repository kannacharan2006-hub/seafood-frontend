...
class _ExportHistoryScreenState extends State<ExportHistoryScreen> {
  ...
  Future<void> fetchExports() async {
    try {
      ...
      ErrorHandler.showError(context, e, onRetry: fetchExports);
    }
  }
  ...
  Future<void> deleteExport(int id) async {
    ...
    try {
      ...
      if (success) {
        ErrorHandler.showSuccess(context, "Invoice deleted successfully");
        fetchExports();
      } else {
        ErrorHandler.showError(context, "Failed to delete invoice");
      }
    } catch (e) {
      if (!mounted) return;
      ErrorHandler.showError(context, e);
